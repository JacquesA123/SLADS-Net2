...
def runEDSSLADSSimulationScript(FolderName,ImageType,ImageExtension,TestingImageSet,TrainingImageSet,SizeImage,c,StoppingPercentage,StoppingThrehsold,Classify,PercentageInitialMask,MaskType,BatchSample,PlotResult,NumSamplesPerIter,Update_ERD,MinWindSize,MaxWindSize,NumSpectra,Folder,NoiseType,Noiselambda,ErrorSpectrumProb, tfclf):
    ...
    from runSLADSOnce import runEDSSLADSSimulationOnce
    from loader import loadOrGenerateInitialMask  
    from computeStopCondRelated import computeBeta
    
    from variableDefinitions import EDSData
    EDSData = EDSData()
    EDSData.initialize(NumSpectra,Folder,NoiseType,Noiselambda,ErrorSpectrumProb)
    ...
    SimulationRun = 1
    ImNum = 1
    runEDSSLADSSimulationOnce(Mask,CodePath,TestingImageSet,SizeImage,StopCondParams,Theta,TrainingInfo,Resolution,ImageType,UpdateERDParams,BatchSamplingParams,SavePath,SimulationRun,ImNum,ImageExtension,PlotResult,Classify,EDSData, tfclf)
